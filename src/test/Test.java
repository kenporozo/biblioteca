/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package test;

import dao.*;

import modelo.*;

/**
 *
 * @author Usuario
 */
public class Test {

    public static void main(String[] args) {

        // Autor autor = new Autor("Yami", "Guerrero");
        //AutorDAO autorDao = new AutorDAO();
        // Autor autor2 = new Autor("Camila", "Carmona");
        //autorDao.insertar(autor);
        // autorDao.insertar(autor2);
        //autorDao.modificar(9,autor);
        //System.out.println(autorDao.buscar(9));
        //autorDao.eliminar(7);
        //autorDao.modificar(6, autor2);
        // System.out.println(autorDao.buscarAutor(3));
        // Categoria cat = new Categoria("Infantil");
        ///catDAO.eliminar(8);
        //EditorialDAO editDao = new EditorialDAO();
        //Editorial editorial = new Editorial("Editorial 6");
        //System.out.println(editDao.buscar(3));
        //IdiomaDAO idiomaDao = new IdiomaDAO();
        //Idioma idioma = new Idioma("Mandarin");
        //idiomaDao.eliminar(6);
        //LibroDAO libroDao = new LibroDAO();
        //Idioma idioma = new Idioma(2, "Ingles");
        //Editorial editorial = new Editorial(2, "Editorial 2");
        //Libro libro = new Libro(3562, "Universidad de JavaScript", 86, "2010", 1200,idioma, editorial);
        //System.out.println(libroDao.getList());
        //libroDao.insertar(libro, 2, 5,3);
        //libroDao.actualizar(libro, 1, 5, 3);
        //System.out.println(libroDao.buscar(10));
        //Telefono telefono = new Telefono("74614526");
        // TelefonoDAO telfDAO = new TelefonoDAO();
        // telfDAO.insertar(telefono);
        //System.out.println(telfDAO.buscar(3));
        // System.out.println(telfDAO.getList());
//       Direccion direccion = new Direccion("Direccion 9");
//       DireccionDAO direc = new DireccionDAO();
//       
//       direc.insertar(direccion);
//        System.out.println(direc.buscar(7));
//        System.out.println(direc.getList());
  // DistribuidorDAO dist = new DistribuidorDAO();
    TelefonoDAO telfDAO = new TelefonoDAO();
    DireccionDAO direcDAO = new DireccionDAO();
//     
   Telefono telefono = (Telefono) telfDAO.buscar(8);
   Direccion direccion = direcDAO.buscar(11);
//     
//    // telfDAO.insertar(telefono);
//     //direcDAO.eliminar(12);
//     
       Distribuidor distribuidor = new Distribuidor(10, "6-5", telefono, direccion, "2021-06-14", 2);
//     //dist.actualizar(10,distribuidor);
//     
//        System.out.println(dist.buscar(10));
        FacturaDAO facturaDAO = new FacturaDAO();
        Factura factura = new Factura(2570, "2021-06-02", "10:35", distribuidor, 2, "Como programar en Java");
        facturaDAO.insertar(factura);
        
        System.out.println(facturaDAO.getList());
    }

}
